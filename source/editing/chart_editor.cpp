...
static const int FFT_ANALYSIS_BINS = 32;
static const int FLUX_HISTORY_SIZE = 20;

static float s_PreviousSpectrum[FFT_ANALYSIS_BINS] = { 0.0f };
static float s_PrevFlux = 0.0f;
static float s_PrevPrevFlux = 0.0f;
static float s_FluxHistory[FLUX_HISTORY_SIZE] = { 0.0f };
static int s_FluxHistoryIndex = 0;
static int s_FluxHistoryCount = 0;

static float s_LastDetectedBeatTime = -1.0f;
static float s_DetectedBPM = 220.0f;
static float s_DetectedBeatInterval = 60.0f / 220.0f;

// 스태틱 지옥

static std::vector<float> s_DetectedBeatTimes;
...
static float ApplyDetectedBeatSnap(float rawTimeSec, float beatIntervalSec, float& outSnapErrorMs) {
    if (!s_DetectedBeatTimes.empty()) {
        float nearestBeatTime = rawTimeSec;
        float bestError = 999999.0f;

        for (float detectedTime : s_DetectedBeatTimes) {
            float error = fabsf(rawTimeSec - detectedTime);

            if (error < bestError) {
                bestError = error;
                nearestBeatTime = detectedTime;
            }
        }

        float bestErrorMs = bestError * 1000.0f;
        ...
        if (s_LastDetectedBeatTime >= 0.0f && s_DetectedBeatInterval > 0.0f) {
            float localIndex = roundf((rawTimeSec - s_LastDetectedBeatTime) / s_DetectedBeatInterval);
            float predictedBeatTime = s_LastDetectedBeatTime + (localIndex * s_DetectedBeatInterval);
            float predictedErrorMs = fabsf(rawTimeSec - predictedBeatTime) * 1000.0f;

            if (predictedErrorMs <= s_QuantizeThresholdMs) {
                outSnapErrorMs = predictedErrorMs;
                return predictedBeatTime;
            }
        }
    }

    return ApplyQuantizeFilter(rawTimeSec, beatIntervalSec, outSnapErrorMs);
}

static void ResetBeatAnalysis() {
    std::fill(std::begin(s_PreviousSpectrum), std::end(s_PreviousSpectrum), 0.0f);
    std::fill(std::begin(s_FluxHistory), std::end(s_FluxHistory), 0.0f);

    s_PrevFlux = 0.0f;
    s_PrevPrevFlux = 0.0f;
    s_FluxHistoryIndex = 0;
    s_FluxHistoryCount = 0;

    s_LastDetectedBeatTime = -1.0f;
    s_DetectedBPM = s_EditorBPM;
    s_DetectedBeatInterval = 60.0f / s_DetectedBPM;

    s_DetectedBeatTimes.clear();
}

static void AnalyzeFmodBeat(float currentSec) {
    if (!s_FftDsp) return;
    ...
    if (fft->numchannels <= 0) return;

    float spectralFlux = 0.0f;

    for (int i = 0; i < FFT_ANALYSIS_BINS; ++i) {
        float currentMagnitude = fft->spectrum[0][i];

        if (currentMagnitude > s_PreviousSpectrum[i]) {
            spectralFlux += currentMagnitude - s_PreviousSpectrum[i];
        }

        s_PreviousSpectrum[i] = currentMagnitude;
    }

    s_FluxHistory[s_FluxHistoryIndex] = spectralFlux;
    s_FluxHistoryIndex = (s_FluxHistoryIndex + 1) % FLUX_HISTORY_SIZE;

    if (s_FluxHistoryCount < FLUX_HISTORY_SIZE) {
        s_FluxHistoryCount++;
    }

    float averageFlux = 0.0f;

    for (int i = 0; i < s_FluxHistoryCount; ++i) {
        averageFlux += s_FluxHistory[i];
    }

    if (s_FluxHistoryCount > 0) {
        averageFlux /= (float)s_FluxHistoryCount;
    }

    float dynamicFluxThreshold = averageFlux * 1.35f + 0.0025f;

    float detectedTime = currentSec - (512.0f / 48000.0f);
    ...
    bool isPeak =
        s_FluxHistoryCount >= 5 &&
        s_PrevFlux > s_PrevPrevFlux &&
        s_PrevFlux >= spectralFlux &&
        s_PrevFlux > dynamicFluxThreshold;

    if (isPeak) {
        if (s_LastDetectedBeatTime < 0.0f || detectedTime - s_LastDetectedBeatTime >= 0.08f) {
            float previousDetectedTime = s_LastDetectedBeatTime;

            s_DetectedBeatTimes.push_back(detectedTime);
            s_LastDetectedBeatTime = detectedTime;

            if (previousDetectedTime >= 0.0f) {
                float interval = detectedTime - previousDetectedTime;

                if (interval >= 0.20f && interval <= 1.20f) {
                    float detectedBPM = 60.0f / interval;

                    while (detectedBPM < 60.0f) {
                        detectedBPM *= 2.0f;
                    }

                    while (detectedBPM > 240.0f) {
                        detectedBPM /= 2.0f;
                    }

                    if (s_DetectedBPM <= 0.0f) {
                        s_DetectedBPM = detectedBPM;
                    } else {
                        s_DetectedBPM =
                            s_DetectedBPM * 0.70f +
                            detectedBPM * 0.30f;
                    ...
                    if (s_DetectedBPM < 60.0f) s_DetectedBPM = 60.0f;
                    if (s_DetectedBPM > 240.0f) s_DetectedBPM = 240.0f;
...
void ChartEditor::HandleInput() {
    ...
    if (s_IsRecording) {
        ...
        if (s_IsAutoRecording && s_FftDsp) {
            ...
            if (FMOD_DSP_GetParameterData(
                ...
                if (fft->numchannels > 0) {
                    ...
                    if (s_IsAudioStarted) {
                        ...
                        if (s_EnergyHistoryCount < ENERGY_HISTORY_SIZE) {
                            s_EnergyHistoryCount++;
                        }
                        ...
                        bool isBurstMode =
                            (s_EnergyHistoryCount >= 5) &&
                            (totalEnergy > avgEnergy * 2.0f) &&
                            (avgEnergy > 0.003f);

                        float dynamicThreshold =
                            isBurstMode
                                ? (avgEnergy * 1.05f + 0.005f)
                                : (avgEnergy * 1.25f + 0.010f);

                        float beatIntervalSec =
                            s_DetectedBeatInterval > 0.0f
                                ? s_DetectedBeatInterval
                                : (60.0f / s_EditorBPM);

                        float bestSnapErrorMs = 0.0f;

                        float nearestBeatTime =
                            ApplyDetectedBeatSnap(
                                currentSec,
                                beatIntervalSec,
                                bestSnapErrorMs
                            );
                        ...
                        bool isOnset =
                            (totalEnergy > dynamicThreshold && energyDelta > 0.02f) ||
                            (isBurstMode && energyDelta > 0.010f);

                        if (isOnset &&
                            (s_LastAutoNoteTime < 0.0f ||
                             (nearestBeatTime - s_LastAutoNoteTime >= minNoteIntervalSec))) {
                            ...
                            if (!duplicate) {
                                bool isLowBpmMode = (s_DetectedBPM < 150.0f);

                                bool isLaurHardcoreKick =
                                    (lowEnergy > midEnergy * 1.30f) &&
                                    (lowEnergy > highEnergy * 1.30f) &&
                                    (totalEnergy > avgEnergy * 1.30f);

                                bool isHighMelody =
                                    (highEnergy > midEnergy * 1.15f) &&
                                    (highEnergy > lowEnergy * 1.15f);

                                if (!isLowBpmMode &&
                                    (isLaurHardcoreKick ||
                                     (isBurstMode && lowEnergy > avgEnergy * 1.70f))) {
                                    ...
                                    bool isFastStream =
                                        ...
                                        (isBurstMode ||
                                         (s_LastAutoNoteTime > 0.0f &&
                                          (nearestBeatTime - s_LastAutoNoteTime <= interval16 * 1.25f)));
                                    ...
                                    if (isLowBpmMode) {
                                        ...
                                        if (targetHand == 0) {
                                            candidateLane =
                                                (s_LastAssignedLane == 0)
                                                    ? 1
                                                    : 0;
                                        } else {
                                            candidateLane =
                                                (s_LastAssignedLane == 2)
                                                    ? 3
                                                    : 2;
                                        }
                                        ...
                                        candidateLane =
                                            PATTERNS[s_ActivePatternType][s_PatternSubIndex];

                                        s_PatternSubIndex =
                                            (s_PatternSubIndex + 1) % 4;

                                        if (s_PatternSubIndex == 0) {
                                            s_ActivePatternType =
                                                (s_ActivePatternType + 1) % 3;
                                        }
                                    ...
                                    if (candidateLane == s_LastAssignedLane &&
                                        ...
                                        for (int offset = 1;
                                            ...
                                            int testLane =
                                                (candidateLane + offset) % LANE_COUNT;
                                    ...
                                    Color noteColor =
                                        (bestSnapErrorMs <= s_QuantizeThresholdMs)
                                            ? SKYBLUE
                                            : ORANGE;
            ...
            if (anyKeyPressed) {
                ...
                float exactInputTime =
                    currentSec - s_InputLatency;

                if (exactInputTime < 0.0f) {
                    exactInputTime = 0.0f;
                }
                ...
                if (!beatAlreadyHasNote) {
                    if (s_ManualPatternIndex < 0 ||
                        s_ManualPatternStep >=
                            (int)EASY_PATTERN_POOL[s_ManualPatternIndex].size()) {

                        std::vector<int> validPatternIndices;

                        for (size_t p = 0;
                             p < EASY_PATTERN_POOL.size();
                             ++p) {
                        ...
                        if (!validPatternIndices.empty()) {
                            s_ManualPatternIndex =
                                validPatternIndices[rand() % validPatternIndices.size()];
                        } else {
                            s_ManualPatternIndex =
                                rand() % EASY_PATTERN_POOL.size();
                        }
                    ...
                    int targetLane =
                        EASY_PATTERN_POOL[s_ManualPatternIndex][s_ManualPatternStep++];
    ...
    if (IsKeyPressed(KEY_Q)) {
        ...
        if (std::filesystem::exists("music")) {
            for (const auto& entry :
                 std::filesystem::directory_iterator("music")) {
    ...
    if (s_IsSpeedInputActive) {
        ...
        if (IsKeyPressed(KEY_ENTER)) {
            if (!s_SpeedInputString.empty()) {
                s_ScrollSpeed =
                    (float)std::stoi(s_SpeedInputString);
            }
    ...
    if (s_IsMusicSelectOpen) {
        ...
        float boxHeight =
            60.f + s_MusicFileList.size() * 45.f;
        ...
        if (IsKeyPressed(KEY_DOWN)) {
            s_SelectedMusicIndex =
                (s_SelectedMusicIndex + 1) %
                (int)s_MusicFileList.size();
        }

        if (IsKeyPressed(KEY_UP)) {
            s_SelectedMusicIndex =
                (s_SelectedMusicIndex - 1 +
                 s_MusicFileList.size()) %
                (int)s_MusicFileList.size();
        }
        ...
        if (IsKeyPressed(KEY_ENTER)) {
            if (!s_MusicFileList.empty() &&
                ...
                s_CurrentMusicPath =
                    s_MusicFileList[s_SelectedMusicIndex];
    ...
    if (IsKeyPressed(KEY_A)) {
        ...
        std::fill(
            std::begin(s_EnergyHistory),
            std::end(s_EnergyHistory),
            0.0f
        );
...
void ChartEditor::Render() {
    ...
    if (s_IsRecording && m_MusicPlayer) {
        float currentSec =
            (float)m_MusicPlayer->GetCurrentPositionMs() /
            1000.0f;

        scrollOffset =
            currentSec * s_ScrollSpeed;
        ...
        float beatPhase =
            fmodf(currentSec, beatInterval) /
            beatInterval;

        if (beatPhase < 0.2f) {
            s_MetronomeFlashAlpha =
                (1.0f - (beatPhase / 0.2f)) *
                0.15f;
        } else {
    ...
    DrawTextEx(
        ...
        editorTexts[0].c_str(),
        Vector2{
            leftVisualizerX + 25.0f,
            leftVisualizerY + 25.0f
        },
        18.0f,
        1.0f,
        RAYWHITE
    ...
    if (s_FftDsp && m_MusicPlayer) {
        ...
        if (FMOD_DSP_GetParameterData(
            ...
            if (fft->numchannels > 0) {
                ...
                float startX =
                    leftVisualizerX + 35.0f;

                float baseY =
                    leftVisualizerY + 520.0f;

                float totalWidth =
                    leftVisualizerW - 70.0f;

                float barWidth =
                    (totalWidth / (float)barCount) -
                    4.0f;
        ...
        DrawRectangleRounded(
            Rectangle{
                leftVisualizerX + 50.0f,
                leftVisualizerY + 180.0f,
                leftVisualizerW - 100.0f,
                260.0f
            },
            0.05f,
            4,
        ...
        for (int k = 0; k < 5; ++k) {
            ...
            DrawLineEx(
                {
                    leftVisualizerX + 80.0f,
                    lineY
                },
                {
                    leftVisualizerX +
                        leftVisualizerW - 80.0f,
                    lineY
                },
                1.0f,
                Fade(SKYBLUE, 0.08f)
        ...
        DrawTextEx(
            ...
            editorTexts[2].c_str(),
            Vector2{
                leftVisualizerX + 85.0f,
                leftVisualizerY + 295.0f
            },
            13.0f,
            1.0f,
            Fade(SKYBLUE, 0.8f)
    ...
    for (float worldY = firstGridY;
        ...
        if (screenY >= 0.0f &&
            screenY <= JUDGMENT_LINE_Y) {

            bool isBarLine =
                (int(roundf(worldY / subGridSize)) % 16) == 0;

            bool isBeatLine =
                (int(roundf(worldY / subGridSize)) % 4) == 0;

            float alpha =
                isBarLine
                    ? 0.35f
                    : (isBeatLine ? 0.18f : 0.06f);
    ...
    if (m_MusicPlayer) {
        ...
        float syncLineScreenY =
            JUDGMENT_LINE_Y -
            (musicWorldY - scrollOffset);

        if (syncLineScreenY >= 0.0f &&
            syncLineScreenY <= JUDGMENT_LINE_Y) {

            DrawLineEx(
                {
                    LANE_START_X,
                    syncLineScreenY - 1.0f
                },
                {
                    LANE_START_X +
                        LANE_AREA_WIDTH,
    ...
    for (const auto& note : s_EditorNotes) {
        float screenNoteY =
            JUDGMENT_LINE_Y -
            (note.posX - scrollOffset);

        if (screenNoteY >= -30.0f &&
            screenNoteY <= JUDGMENT_LINE_Y + 30.0f) {

            float noteX =
                LANE_X_COORDS[note.lane];
    ...
    for (int i = 0; i < LANE_COUNT; ++i) {
        ...
        if (i < 4 &&
            (s_LanePulseTimers[i] > 0.0f ||
             s_LanePulseTimers[i + 2] > 0.0f)) {
    ...
    DrawTextEx(
        s_SuitFont,
        chartSpeedDisplay.c_str(),
        Vector2{
            leftVisualizerX + 380.0f,
            leftVisualizerY + 625.0f
        },
        14.0f,
        1.0f,
