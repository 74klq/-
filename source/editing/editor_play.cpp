...
static MusicExecute::MusicPlayer1* s_MusicPlayerRef = nullptr;
...
void EditorPlay::Update(bool& isPlaying) {
    ...
    if (s_MusicPlayerRef != nullptr) {
        float currentSec = (float)s_MusicPlayerRef->GetCurrentPositionMs() / 1000.0f;
        currentWorldY = currentSec * s_EditorScrollSpeed;
    }

    for (size_t i = 0; i < playNotes.size(); ++i) {
        if (i < s_NoteActiveStates.size() && s_NoteActiveStates[i]) {
            float noteWorldY = playNotes[i].posX;
            float screenNoteY = JUDGMENT_LINE_Y - (noteWorldY - currentWorldY);
    ...
    if (inputLane != -1) {
        ...
        for (size_t i = 0; i < playNotes.size(); ++i) {
            ...
            float noteWorldY = playNotes[i].posX;
            float diff = fabsf(noteWorldY - currentWorldY);
...
void EditorPlay::Draw() {
    ...
    float centerlineX = centerX;
    float centerlineY = centerY;
    DrawLineEx({ centerlineX, centerlineY }, { centerlineX, centerlineY - 8.0f }, 1.5f, Fade(WHITE, 0.7f));
    DrawLineEx({ centerlineX, centerlineY }, { centerlineX + 8.0f, centerlineY }, 1.2f, Fade(WHITE, 0.5f));
