...
bool ChartSave::LoadFromJSON(const char* filename, std::string& outMusicPath, float& outSpeed, std::vector<SaveNoteData>& outNotes) {
    ...
    size_t speedPos = content.find("\"speed\"");
    if (speedPos != std::string::npos) {
        size_t colonSpeed = content.find(":", speedPos);
        if (colonSpeed != std::string::npos) {
            outSpeed = std::stof(content.substr(colonSpeed + 1));
        }
    }
...
void ChartSave::HandleChartInput(const std::string& musicPath, float speed, const std::vector<SaveNoteData>& notes, std::string& outMusicPath, float& outSpeed, std::vector<SaveNoteData>& outNotes, bool& fileLoaded) {
    ...
    if (!s_IsLoadOpen && IsKeyPressed(KEY_ZERO) && s_KeyCooldown == 0) {
        if (!s_LastLoadedFilename.empty()) {
            SaveToJSON(s_LastLoadedFilename.c_str(), musicPath, speed, notes);
            s_KeyCooldown = 10;
        } else {
    ...
    if (s_IsSaveOpen) {
        ...
        if (IsKeyPressed(KEY_ENTER)) {
            if (!s_InputString.empty()) {
                ...
                SaveToJSON(filename.c_str(), musicPath, speed, notes);
                s_LastLoadedFilename = filename;
        ...
        for (size_t i = 0; i < charts.size(); ++i) {
            ...
            if (CheckCollisionPointRec(mousePos, itemBox)) {
                if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                    if (LoadFromJSON(charts[i].c_str(), outMusicPath, outSpeed, outNotes)) {
                        fileLoaded = true;
                        s_IsLoadOpen = false;
                        s_KeyCooldown = 10;
                    }
