...
namespace MusicExecute {
    ...
    public:
        ...
        bool Initialize(AudioManager& audioManager) {
            std::string musicName = "Music2";
            std::string musicPath = "music/kaleidoscope.ogg";
        ...
        bool InitializeWithCustomPath(AudioManager& audioManager, const std::string& musicName, const std::string& musicPath) {
            if (!audioManager.CreateStream(musicName, musicPath)) {
                return false;
            }
        ...
        void Play(AudioManager& audioManager, int startWaitFrames = 0) {
            ...
            FMOD_CHANNEL* channel = audioManager.PlaySoundWithDelay("Music2", 0);
